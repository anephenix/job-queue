class Queue {
	...
	async count(keyType) {
		const count = await this.redis.llenAsync(this.subQueueKeys[keyType]);
		return count;
	}

	async counts() {
		const available = await this.count('available');
		const processing = await this.count('processing');
		const failed = await this.count('failed');
		const completed = await this.count('completed');
		return {
			available,
			processing,
			failed,
