class Queue {
	...
	async complete(job) {
		await this.callHook('complete', 'pre', job);
		const payload = JSON.stringify(job);
		await this.redis.lremAsync(this.subQueueKeys.processing, 1, payload);
		await this.redis.rpushAsync(this.subQueueKeys.completed, payload);
		return await this.callHook('complete', 'post', job);
	}

	async fail(job) {
		await this.callHook('fail', 'pre', job);
		const payload = JSON.stringify(job);
		await this.redis.lremAsync(this.subQueueKeys.processing, 1, payload);
		await this.redis.rpushAsync(this.subQueueKeys.failed, payload);
		return await this.callHook('fail', 'post', job);
	}
